...
#[derive(Clone)]
struct KeyRemapperCallbacks {
    on_init_args:
        Arc<dyn for<'a, 'b> Fn(clap::App<'a, 'b>) -> clap::App<'a, 'b> + Send + Sync + 'static>,
    on_args_parsed: Arc<dyn Fn(&clap::ArgMatches) + Send + Sync + 'static>,

    on_start: Arc<dyn Fn(&KeyRemapper) + Send + Sync + 'static>,

    on_devices_detected: Arc<dyn Fn(&KeyRemapper, &[evdev::EvdevDevice]) + Send + Sync + 'static>,
    on_devices_not_found: Arc<dyn Fn(&KeyRemapper) + Send + Sync + 'static>,
    on_devices_lost: Arc<dyn Fn(&KeyRemapper) + Send + Sync + 'static>,

    on_stop: Arc<dyn Fn(&KeyRemapper) + Send + Sync + 'static>,

    on_events: Arc<
        dyn Fn(&KeyRemapper, &evdev::EvdevDevice, &[evdev::InputEvent]) + Send + Sync + 'static,
    >,
    on_event:
        Arc<dyn Fn(&KeyRemapper, &evdev::EvdevDevice, &evdev::InputEvent) + Send + Sync + 'static>,
}
...
pub struct KeyRemapperConfiguration {
    ...
    device_name_regex: String,
    id_regex: String,

    device_name_regex_re: Option<Regex>,
    id_regex_re: Option<Regex>,

    use_non_keyboard: bool,
    grab_devices: bool,
    write_to_uinput: bool,
    uinput_events: EventsDescriptor,
    global_lock_name: String,
    uinput_device_name_suffix: String,

    callbacks: Arc<RwLock<KeyRemapperCallbacks>>,
}
...
impl KeyRemapperConfiguration {
    ...
    pub fn on_args_parsed<F: Fn(&clap::ArgMatches) + Send + Sync + 'static>(
        &mut self,
        callback: F,
    ...
    pub fn on_start<F: Fn(&KeyRemapper) + Send + Sync + 'static>(
        &mut self,
        callback: F,
    ) -> &mut KeyRemapperConfiguration {
        {
    ...
    pub fn on_devices_detected<
        F: Fn(&KeyRemapper, &[evdev::EvdevDevice]) + Send + Sync + 'static,
    >(
        &mut self,
        callback: F,
    ) -> &mut KeyRemapperConfiguration {
    ...
    pub fn on_devices_not_found<F: Fn(&KeyRemapper) + Send + Sync + 'static>(
        &mut self,
        callback: F,
    ) -> &mut KeyRemapperConfiguration {
        {
    ...
    pub fn on_devices_lost<F: Fn(&KeyRemapper) + Send + Sync + 'static>(
        &mut self,
        callback: F,
    ) -> &mut KeyRemapperConfiguration {
        {
    ...
    pub fn on_stop<F: Fn(&KeyRemapper) + Send + Sync + 'static>(
        &mut self,
        callback: F,
    ) -> &mut KeyRemapperConfiguration {
        {
    ...
    fn set_defaults(&mut self) -> &mut KeyRemapperConfiguration {
    ...
    fn update_regexes(&mut self) {
        self.device_name_regex_re =
            Some(Regex::new(&self.device_name_regex).expect("Invalid regex detected"));
        self.id_regex_re = Some(Regex::new(&self.id_regex).expect("Invalid regex detected"));
    }

    fn callbacks_cloned(&self) -> KeyRemapperCallbacks {
